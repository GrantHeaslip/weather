...
const citiesBySlug = require('../data/citiesBySlug');
const climaconMappings = require('../data/climaconMappings');
const impliedPopConditions = require('../data/impliedPopConditions');
...
function processResponse(weatherAtomObject) {
    ...
    weather.locationName = citiesBySlug[locationId] || 'Unknown location';
...
function parseCurrent(conditionsString) {
    ...
    for (var i = 0; i < segments.length; i++) {
        ...
        if (temperatureRegExp.test(segments[i])) {
            conditions.temp = temperatureRegExp.exec(segments[i])[1];
        }
    }
